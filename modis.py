...
#import geoimagine.kartturmain.VectorLayer
...
class ProcessModis:
    ...
    def __init__(self, process, session, verbose):
        ...
        self.session = session 
        print ('        ProcessModis',self.process.proc.processid)        
        #direct to subprocess
        if self.process.proc.processid.lower() == 'searchdatapool':
            self._SearchDataPool()
            
        elif self.process.proc.processid.lower() == 'modissearchtodb':
            self._ModisSearchToDB()
            
        elif self.process.proc.processid.lower() == 'linkdefaultregionstomodis':
            self._LinkDefaultRegionsToMODIS()
            
        elif self.process.proc.processid.lower() == 'linkuserregiontomodis':
            self._LinkUserRegionToMODIS()
            
        elif self.process.proc.processid.lower() == 'linkinternaltomodis':
        ...
        elif self.process.proc.processid.lower() == 'downloadmodissingletile':
            self._downloadModisSingleTile() 
            
        elif self.process.proc.processid.lower() == 'downloadmodisregiontiles':
            self._downloadModisRegionTiles() 
            
        elif self.process.proc.processid.lower() == 'explodemodisregion':
            self._ExplodeMODISRegion()
            
        elif self.process.proc.processid.lower() == 'explodemodissingletile':
            self._ExplodeMODISSingleTile()
         
        elif self.process.proc.processid.lower() == 'checkmodissingletile':
            self._CheckModisSingleTile()   
            
        elif self.process.proc.processid.lower() == 'checkmodisregion':
            self._CheckModisRegion() 
            
        elif 'resamplespatial' in self.process.proc.processid.lower():
            self._ResampleSpatial() 
            
        elif 'tileregiontomodis' in self.process.proc.processid.lower():
            self._TileRegionToModis() 
            
        elif self.process.proc.processid.lower() == 'mosaicmodis':
            self._MosaicModis() 

        else:
            exitstr = 'Exiting, processid %(p)s missing in ProcessModis' %{'p':self.process.proc.processid}
            exit(exitstr)
    ...
    def _GetModisTilesDict(self):
        '''
        '''
        withintiles = self.session._SelectModisRegionTiles({'regionid':self.process.params.mask})
    ...
    def _GetSystemDefRegions(self):
        ...
        for rec in recs:            
            ...
            queryD['regionid'] = regionid
            #SELECT * FROM system.layers WHERE compid = 'defaultregions_roi' and regionid = 'se';
            ...
            #Create the layer
            regionLayer = VectorLayer(comp, locusD, datumD, self.process.srcpath)
